...
// === Live total preview (Add Sale modal) ===
// Reads whatever is currently in the item rows + discount field and
// reflects Subtotal / Discount / Total in real time, so the cashier can
// see the numbers before hitting Save. Purely visual — saveManualSale()
// still does its own authoritative calculation on submit.
function updateSaleTotalPreview() {
  const subtotalEl = document.getElementById("saleSubtotalPreview");
  const discountEl = document.getElementById("saleDiscountPreview");
  const totalEl = document.getElementById("saleTotalPreview");
  ...
  const discountAmountInput = document.getElementById("saleDiscountAmount");
  const discountAmount = parseFloat(discountAmountInput?.value) || 0;
  const total = Math.max(0, subtotal - discountAmount);

  subtotalEl.textContent = `RM${subtotal.toFixed(2)}`;
  discountEl.textContent = `-RM${discountAmount.toFixed(2)}`;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function setupAddSaleModal() {
    ...
    // Live total preview: recompute whenever qty/price change inside the
    // items list, or the discount amount changes. Delegated on the
    // container so it also covers rows added later by addItemRow().
    ...
    const discountInput = document.getElementById("saleDiscountAmount");
    if (discountInput) {
      discountInput.addEventListener("input", updateSaleTotalPreview);
    }
  }
  ...
  function addItemRow() {
    ...
    row.innerHTML = `
      ...
      <input type="number" min="0" step="0.01" class="item-price" placeholder="0.00" />
...
// === viewReceipt ===
// FIX: was classList.add/remove("hidden"/"is-open") — that toggle wasn't
// actually showing the modal. Switched to the same inline style.display
// toggle used successfully in Order Management's receipt modal.
